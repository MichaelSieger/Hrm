...
public class TargetPerformanceComparisonImageParser {
    ...
    public String parse(String path, Collection<Photo> pictures) throws IOException {
    ...
    private void parseOneImageLine() {
        buffer.append(prop.getProperty("targetperformancecomparison.image.header.oneimage")
                .replace(IMAGE_HEADER_ONE, pics[pics.length - 1].getLabel()));
        this.appendNewLine();
        buffer.append(prop.getProperty("targetperformancecomparison.image.row.oneimage").replace(
                IMAGE_ONE, pics[pics.length - 1].getName()));
        this.appendNewLine();
    }
    ...
    private void parseTwoImagesLine(int i) {
        ...
        buffer.append(prop.getProperty("targetperformancecomparison.image.row.twoimage")
                .replace(IMAGE_ONE, pics[i - 1].getName()).replace(IMAGE_TWO, pics[i].getName()));
        this.appendNewLine();
    }
