...
public class SchemeComposite extends Composite {
     ...
     * @param style
     */
    public SchemeComposite(Composite parent, int style, ComponentService componentsService,
            Scheme scheme) {
        super(parent, style);
    ...
    private void createControls() {
         ...
         */
        try {
            setRenderedComponents(Collections2.transform(componentsService.findAll(),
    ...
    private void copyScheme() {
         ...
         * shellProvider.getShell(), context, plant); ContextInjectionFactory.inject(dialog,
         * context);
         * 
         * if (dialog.open() != Window.OK) { return; }
         * 
         * try { Scheme scheme = schemeService.copy(dialog.getScheme(), plant);
         * modifyScheme(scheme); } catch (DatabaseException e) {
         * LOG.error("Error during scheme copy.", e);
         * MessageDialog.openError(shellProvider.getShell(), "Copy Error",
         * "Could not copy the requested scheme."); } catch (IOException e) {
         * LOG.error("Error during scheme copy.", e);
         * MessageDialog.openError(shellProvider.getShell(), "Copy Error",
    ...
    private void importScheme() {
         ...
         * shellProvider.getShell(), context); ContextInjectionFactory.inject(dialog, context);
         * 
         * if (dialog.open() != Window.OK) { return; }
         * 
         * try { Scheme scheme = schemeService.copy(dialog.getScheme(), plant);
         ...
         * "Could not import the requested scheme."); } catch (IOException e) {
    ...
    private void saveScheme() {
        ...
        try {
            /*
             * if (currentScheme.getPlant().isPresent()) { schemeService.update(currentScheme,
             * schemeComps); System.out.println("after update"); } else {
             * schemeService.insert(plant, schemeComps); System.out.println("after insert"); }
             */
    ...
    public Optional<Plant> getSelectedPlant() {
