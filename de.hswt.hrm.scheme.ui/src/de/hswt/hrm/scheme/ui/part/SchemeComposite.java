...
public class SchemeComposite extends Composite {
	...
	private List<SchemeGridItem> toSchemeGridItems(
		...
		for (SchemeComponent c : sc) {
			l.add(new SchemeGridItem(ComponentConverter.convert(
					this.getDisplay(), c.getComponent()), c));
		}
		return l;
	}
	...
	private void editAttribute(SchemeGridItem item) {
		...
		try {
			Collection<Attribute> attributes = componentsService
					.findAttributesByComponent(item.getRenderedComponent()
							.getComponent());
			if (attributes.isEmpty()) {
				MessageDialog.openError(shellProvider.getShell(),
						"No Atrributes", "The selected Component "
								+ item.getRenderedComponent().getComponent()
										.getName()
	...
	private void initGridDropTargetListener() {
		gridListener = new GridDropTargetListener(grid, this, schemeService,
				componentsService);
	...
	private void initGridDND() {
		gridDragListener = new GridDragListener(grid, schemeService,
				componentsService);
