...
public class PlantWizardPageOne extends WizardPage {
	...
	private RegexValidator numberVal = new RegexValidator(
			"[0-9]+([.,]{1}[0-9]+)?");
	...
	private void loadConstructionYears() {
		...
		Combo constYearCombo = (Combo) XWT.findElementByName(container,
				"constructionYear");
	...
	public HashMap<String, Text> getOptionalWidgets() {
		HashMap<String, Text> widgets = new HashMap<String, Text>();
		widgets.put("manufactor",
				(Text) XWT.findElementByName(container, "manufactor"));
		widgets.put("type", (Text) XWT.findElementByName(container, "type"));
		widgets.put("airPerformance",
				(Text) XWT.findElementByName(container, "airPerformance"));
		widgets.put("motorPower",
				(Text) XWT.findElementByName(container, "motorPower"));
		widgets.put("ventilatorPerformance", (Text) XWT.findElementByName(
				container, "ventilatorPerformance"));
		widgets.put("motorRPM",
				(Text) XWT.findElementByName(container, "motorRPM"));
		widgets.put("current",
				(Text) XWT.findElementByName(container, "current"));
		widgets.put("voltage",
				(Text) XWT.findElementByName(container, "voltage"));
	...
	public HashMap<String, Combo> getOptionalCombos() {
		...
		combos.put("constructionYear",
				(Combo) XWT.findElementByName(container, "constructionYear"));
		return combos;
	}
	...
	public boolean isPageComplete() {
		...
		// Sorted array
		Text[] optArray = { optWidgets.get("airPerformance"),
				optWidgets.get("motorPower"),
				optWidgets.get("ventilatorPerformance"),
				optWidgets.get("motorRPM"), optWidgets.get("current"),
	...
	private boolean checkValidity(Text textField) {
		...
		boolean isInvalidNumber = (textFieldName.equals("airPerformance")
				...
				|| textFieldName.equals("motorRPM")
				|| textFieldName.equals("current") || textFieldName
					.equals("voltage"))
	...
	private void addListenerToWidgets(HashMap<String, Text> widgets,
